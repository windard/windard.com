...
在 python3 中还有一个新的关键词叫 `nonlocal` 😂，no local 不就是 global 么、? 但是其实有很大差异的。
1. `nonlocal` 不能在最外层，也不能在一级函数中，只能在二级函数中. 在最外层，那就是 global 的，在一级函数中，往外就到了 global 。
2. `nonlocal` 只能往外一层，再 `nonlocal` 再往外一层。
