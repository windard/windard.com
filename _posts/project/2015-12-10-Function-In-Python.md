...
|函数                   |说明                                             |
|:-----:                |:------:                                         |
|int(x=0[,base=10])     |将其他进制的字符串或数转化为十进制整数，若为十进制浮点数，则表示下取整|
...
- `round(number[, ndigits=0])` 将一个浮点数转换为取整，并不是四舍五入，而是`ROUND_HALF_EVEN` 策略，取最近的偶数，返回一个 ndigits 位的浮点数
- `pow(x, y[, z]) = (x**y) % z` 求幂
...
#### input和raw_input
input()相当于 `eval(raw_input())`，input()和raw_input()都是标准输入函数。
...
链式比较即 `5 > 4 > 3` 或者 `5 < 4 < 3` 类似于这种，在某些语言中得到的结果与正常人的想法相反，比如 C语言，结果是 `false` 和 `true`。

在 Python 中终于恢复正常了，相当于 `5 > 4 and 4 > 3` 和 `5 < 4 and 4 < 3` ，与正常的想法一致，结果是 `True` 和 `False`

#### python 没有三元运算符

如果学过其他的编程语言的话，三元运算符 `?:` 还是在很多地方很有帮助的，但是非常遗憾的是 python 并不支持原生的三元运算符，如果你需要这样的效果的话，可以用另一种方法。
...
```
>>> a='2017 04        20.0    27.0    13.0       79.8    84.8    74.8'
>>> a.split()
['2017', '04', '20.0', '27.0', '13.0', '79.8', '84.8', '74.8']
>>> [n for n in a.split(' ') if n]
['2017', '04', '20.0', '27.0', '13.0', '79.8', '84.8', '74.8']
```
