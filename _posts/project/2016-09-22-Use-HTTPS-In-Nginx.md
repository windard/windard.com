...
检测一下是否配置正确，注意 
1. `ssl on;` 后面有一个分号。
2. `ssl_certificate /etc/nginx/server.crt;` 这是证书文件，如果是申请的证书，则为完整证书链文件
3. `ssl_certificate_key /etc/nginx/server.key;` 这是私钥文件，这两个文件都可以用 pem 结尾。
...
可以使用 [Certificate Search](https://crt.sh/) 网站查询证书。

现在一般用 Let's Encrypt 的证书，获取其比较方便的两个的方式 [acme.sh](https://github.com/acmesh-official/acme.sh) 和 [来此加密](https://letsencrypt.osfipin.com/)

### HTTPS 增强配置

在 nginx 上配置 HTTPS，除了必备的 ssl 三件套之外，还有一些其他的配置可以加上。
...
server {
        ...
        # 支持的加密算法版本，任何一个版本的 TLS 都比 SSLv3 要安全
        ssl_protocols TLSv1 TLSv1.1 TLSv1.2 TLSv1.3;

        # 选择加密套件，不同的浏览器所支持的套件（和顺序）可能会不同。这里指定的是OpenSSL库能够识别的写法, 拒接 MD5,RC4,DHE 等过时不安全的算法
        ...
        # 客户端可以重用会话缓存中ssl参数的过期时间，系统默认5分钟太短了，可以设成30m即30分钟甚至4h。
