...
139. vim 有时插入数据因为 tab 的原因会有问题，在插入最后的时候可以这样 `cat >> test.py <<EOF
> print "hello world"
>
> EOF
`
...
15. `inner join` 就是 `join`,只展示两张表中都有的行，如果存在不匹配，则不展示

16. `left join` 就是，返回左表所有的行

17. `right join` 就是，返回右表所有的行

18. `full join`,MySQL 不支持，SQL Server 才有
